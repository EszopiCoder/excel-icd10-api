...
Public Sub OpenSheetQuery()

    ' Open userform
    formQueryICD.Show vbModeless
    ...
    If rng.Columns.Count <> 2 Then
        formQueryICD.Hide
        MsgBox "Columns is not equal to 2", vbInformation
        Exit Sub
    End If
    
    For i = 1 To rng.Rows.Count
        With formQueryICD.listSearch
            .AddItem
            .List(.ListCount - 1, 0) = rng.Rows(i).Columns(1).Text
            .List(.ListCount - 1, 1) = rng.Rows(i).Columns(2).Text
        End With
    Next i
    
End Sub

Public Sub OpenQueryForm()
    formQueryICD.Show vbModeless
End Sub
...
Public Sub MsgBoxDict(objDict As Object, _
    Optional boolModal As Boolean = False)
